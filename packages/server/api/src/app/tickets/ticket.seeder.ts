...
async function seedTickets() {
    const ticketCategoryRepo = databaseConnection().getRepository(TicketCategoriesEntity);
    ...
    for (let i = 0; i < 10; i++) {
        const randomCategory = await ticketCategoryRepo.createQueryBuilder('category')
            .orderBy('RANDOM()')
            .limit(1)
            .getOne();

        const randomUser = await userRepo.createQueryBuilder('user')
            .orderBy('RANDOM()')
            .limit(1)
            .getOne();

        const randomSchool = await schoolRepo.createQueryBuilder('school')
            .orderBy('RANDOM()')
            .limit(1)
            .getOne();

        if (!randomCategory || !randomUser || !randomSchool) {
            throw new Error('Failed to find random related entities for ticket seeding.');
        }
