...
export const ticketsService = {
  async create({ title, description, categoryId, schoolId }: CreateParams): Promise<Ticket> {
  ...
  async list({ schoolId, cursor, limit }: ListParams): Promise<SeekPage<Ticket>> {
    ...
    let query = ticketsRepo().createQueryBuilder('ticket').where({ schoolId });

    const { data, cursor: newCursor } = await paginator.paginate(query);
    return paginationHelper.createPage<Ticket>(data, newCursor);
  },
  async update({ id, schoolId, status, categoryId }: UpdateParams): Promise<Ticket> {
    await ticketsRepo().update({ id, schoolId }, {
